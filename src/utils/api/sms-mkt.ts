...
const createClient = () => {
  ...
  ax.interceptors.request.use((request: any) => {
    request.url = `${process.env.SMS_MKT_API_HOST}/${request.url}`
    request.headers.common['api_key'] = process.env.SMS_MKT_API_KEY
    request.headers.common['secret_key'] = `${process.env.SMS_MKT_API_SECRET}`
    ...
    if (request.data) {
      if (!(request.headers['Content-Type'] === ContentType.FORMDATA)) {
        ...
        request.headers = request.data.getHeaders()
      }
    }
...
export const smsMktClient = createClient()
export const smsMktApiWraper = async (method: Promise<AxiosResponse>) => {
  try {
